...
- [x] OG(Open Graph) 메타태그 포함 공유 URL 생성 (`GET /share/{id}`)
- [x] 카드에 포함할 내용: 날짜, 주차, 리포트 한 줄 요약, 서비스 브랜딩
- [x] 공개 공유 카드 이미지 (`GET /share/{id}/card.png`) — 인증 불필요
- [x] `app.html` 분석 완료 시 공유 버튼 + 링크 복사 기능
...
- [x] 프론트엔드 전략 결정 → **Thymeleaf 유지** (서버사이드 렌더링, 빠른 개발)
- [x] 회원가입 / 로그인 페이지 (`app.html`)
- [x] 이미지 업로드 페이지 (`app.html` — 드래그앤드롭)
- [x] 분석 결과 페이지 (`app.html` — 리포트 + 공유 버튼)
- [x] 성장 타임라인 페이지 (`timeline.html`)
